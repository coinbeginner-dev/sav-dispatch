...
// Ligne d'export pour "toute la base" (listerTousLesTickets) : contrairement
// à ligneExport, la donnée est déjà par ticket (pas par jour affiché) — pas
// de map assign/statuts/reports à croiser, tout vient du ticket lui-même.
export function ligneExportBase(t) {
  const d = t.dernierStatut;
  return {
    ref: t.ref,
    client: t.client,
    contact: t.contact + (t.contact_manual ? ' (corrigé)' : ''),
    msan: t.msan,
    equipe: t.assigned_tech || '',
    statut: t.status === 'clos' ? 'Clôturé' : 'Ouvert',
    dernierRetour: d ? (LIBELLE[d.statut] || d.statut) : '',
    motif: d?.motif || '',
    precision: d?.texte || '',
    dernierRetourLe: d?.day || '',
    premierVu: t.first_seen,
    dernierVu: t.last_seen,
    joursVus: t.days_seen,
    clotureLe: t.closed_on || '',
    envoye: t.envoye_le ? new Date(t.envoye_le).toLocaleString('fr-FR') : '',
    historique: historiqueTexte(t.historique),
    cle: t.status === 'clos' ? 'clos' : (d ? statutCouleur({ hors_dispatch: false, delai: 0 }, d) : 'vert'),
  };
}

export function lignesExportBase(tickets) {
  return tickets.map(ligneExportBase);
}
