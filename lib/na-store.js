...
export async function loadInitial() {
  try {
    ...
    if (s.db && Array.isArray(s.teams)) {
      ...
      return {
        db: true, teams: s.teams, sro: s.sro,
        commandes: d.db ? d.commandes || [] : [],
        historique: d.db ? d.historique || {} : {},
      };
    }
  } catch {}
  return { db: false, teams: [], sro: [], commandes: [], historique: {} };
}

export async function saveSettings(db, teams, sro) {
  if (!db) return { teams, sro };
  const r = await fetch('/api/na/settings', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ teams, sro }),
  });
