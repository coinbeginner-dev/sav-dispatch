...
export async function saveUpload(day, tickets) {
  ...
  // Arbitrage : tout ticket deja renseigne un jour precedent sort du dispatch.
  ...
  if (t.length) {
    await sql`
      with dernier as (
        ...
        from ticket_days
        where day < ${day}::date and statut is not null
      ...
      from dernier
      where d.day = ${day}::date and d.ref = dernier.ref and d.statut is null`;
  }
...
export async function planifier(refs, day, dedans = true) {
  ...
  if (!list.length) return { ok: true, updated: 0 };
  const rows = await sql`
    update ticket_days set hors_dispatch = ${!dedans}
    where day = ${day}::date and ref = any(${list}::text[])
    returning ref`;
  return { ok: true, updated: rows.length };
}
