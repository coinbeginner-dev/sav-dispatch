...
export async function saveUpload(day, tickets, { clore = false } = {}) {
  ...
  // Trace explicitement cette clôture automatique dans l'historique — mais
  // seulement quand le ticket n'a jamais été réellement confirmé (aucun
  // retour, ou dernier retour encore en cours type blocage/planifié). Sans
  // ça, un ticket jamais traité par un technicien se retrouve "Clôturé" avec
  // un historique vide, indiscernable d'une intervention réellement
  // terminée. Si un "Fait"/"Clos" existe déjà, on ne rajoute rien : ce
  // serait redondant et masquerait la vraie confirmation terrain.
  // Source 'systeme' pour ne pas se confondre avec le terrain/l'orienteur.
  if (closed.length) {
    await sql`
      with dernier as (
        select distinct on (ref) ref, statut from ticket_notes order by ref, cree_le desc
      )
      insert into ticket_notes (ref, day, statut, motif, texte, source)
      select u.ref, ${day}::date, 'clos',
             'Clôturé automatiquement — absent du fichier déclaré complet', null, 'systeme'
      from unnest(${closed.map((c) => c.ref)}::text[]) as u(ref)
      left join dernier d on d.ref = u.ref
      where d.ref is null or d.statut not in ('fait', 'clos')`;
  }

  // clore=true veut dire "ce fichier EST la liste complète d'aujourd'hui" :
  // toute ligne ticket_days du jour qui n'est pas dans ce fichier doit
  // disparaître du dispatch actif, même si elle a été créée plus tôt le même
  // jour par un envoi précédent sans clore=true (reconduction automatique).
  // Sans ça, un ticket déjà clôturé ci-dessus continuait quand même
  // d'apparaître dans getDay(day) — la clôture ne suffit pas, il faut aussi
  // retirer la ligne du jour, que getDay (join sur ticket_days) affiche.
  if (clore) {
    await sql`
      delete from ticket_days
      where day = ${day}::date and ref <> all(${col('ref')}::text[])`;
  }
...
export async function rechercheGlobale(terme) {
  ...
  const [historique, dernierStatut] = await Promise.all([
    ...
    // Source ticket_notes (pas ticket_days) : c'est le seul journal qui
    // n'est jamais nettoyé (les lignes ticket_days du jour peuvent être
    // retirées par clore=true) et qui porte aussi les clôtures automatiques
    // système, pas seulement les statuts posés par un technicien/l'orienteur.
    sql`
      select distinct on (ref) ref, statut, motif, texte, day::text as day
      from ticket_notes
      where ref = any(${refs}::text[])
      order by ref, cree_le desc`,
  ]);
...
export async function listerTousLesTickets() {
  ...
  const [historique, dernierStatut] = await Promise.all([
    getHistoriqueTickets(refs),
    // Source ticket_notes (voir rechercheGlobale) : capture aussi les
    // clôtures automatiques système, pas seulement les statuts terrain.
    sql`
      select distinct on (ref) ref, statut, motif, texte, day::text as day
      from ticket_notes
      where ref = any(${refs}::text[])
