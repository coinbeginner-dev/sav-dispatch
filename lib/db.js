...
export async function saveUpload(day, tickets) {
  ...
  //
  // Cas "fait" : deja declare traite un jour precedent -> cloture automatique,
  // sans passer par la liste a arbitrer (reservee aux cas encore en cours).
  // Il compte simplement dans le clos du dashboard.
  ...
  if (t.length) {
    ...
    const clotures = await sql`
      ...
      where d.day = ${day}::date and d.ref = dernier.ref and not d.arbitrage_decide
        and dernier.statut = 'fait'
      returning d.ref, d.motif, d.texte`;

    if (clotures.length) {
      await sql`
        insert into ticket_notes (ref, day, statut, motif, texte, source)
        select u.ref, ${day}::date, 'fait', nullif(u.motif, ''), nullif(u.texte, ''), 'orienteur'
        from unnest(
...
export const STATUTS = ['fait', 'planifie', 'blocage'];
...
//   'cloturer'  → il n'ira chez personne : on acte qu'il est traité et il
//                 quitte la liste d'arbitrage au lieu d'y traîner chaque jour.
// Dans les deux cas la décision est mémorisée et survit aux rechargements.
export async function arbitrer(refs, day, decision = 'planifier') {
  await ensureSchema();
  const sql = db();
  const list = (Array.isArray(refs) ? refs : [refs]).filter(Boolean);
  if (!list.length) return { ok: true, updated: 0 };
  ...
  if (decision === 'cloturer') {
    const rows = await sql`
      update ticket_days set
        ...
        arbitrage_decide = true,
        statut = 'fait',
        motif = coalesce(nullif(motif, ''), nullif(arbitrage_motif, ''), 'Clôturé sans intervention'),
        texte = nullif(arbitrage_texte, ''),
        source = 'orienteur',
        statut_at = ${maintenant}::timestamptz
      where day = ${day}::date and ref = any(${list}::text[])
