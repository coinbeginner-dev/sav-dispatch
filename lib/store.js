...
// compteur de jours. `clore` (coché par l'orienteur : "ce fichier est la
// liste complète du reste à faire") ferme les tickets absents du fichier ;
// par défaut un fichier plus petit n'efface jamais ce qui reste en cours.
export async function pushUpload(db, tickets, zones, techs, clore = false) {
  if (db) {
    const r = await fetch('/api/tickets', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ day: today(), tickets, clore }),
    });
...
// sélection WhatsApp) — pour garder une trace de la dernière date d'envoi.
export async function pushEnvoye(db, refs) {
  if (!db) return;
  await fetch('/api/tickets', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ refs, envoye: true }),
  });
}

// Recherche dans tout l'historique (pas seulement le jour affiché) — par
// n° de ticket ou client. Sert à retrouver un ticket déjà clôturé pour le
// réouvrir, par exemple quand IAM le renvoie pour correction.
export async function rechercheGlobale(db, terme) {
  if (!db || !terme?.trim()) return [];
  const r = await fetch(`/api/tickets?recherche=${encodeURIComponent(terme.trim())}`, { cache: 'no-store' });
  const d = await r.json();
  return d.db ? d.resultats || [] : [];
}

// Tous les tickets jamais suivis en base, pas seulement ceux du dispatch
// actif du jour — pour l'export "toute la base" (utile depuis que clore=false
// par défaut : des tickets encore ouverts peuvent être absents du dispatch
// affiché un jour donné sans pour autant avoir disparu de la base).
export async function listerTousLesTickets(db) {
  if (!db) return [];
  const r = await fetch('/api/tickets?tout=1', { cache: 'no-store' });
  const d = await r.json();
  return d.db ? d.resultats || [] : [];
}

// Réouvre un ticket déjà clôturé : redevient actif pour le jour affiché,
// prêt à être réassigné/reposé, sans attendre qu'un futur fichier le
// fasse réapparaître (ce qui repasserait par l'ancien statut via l'arbitrage).
export async function pushReouvrir(db, refs, jour = today()) {
  if (!db) return;
  await fetch('/api/tickets', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ refs, reouvrir: true, day: jour }),
  });
}

// Corrige le numéro de contact (ex : IAM en donne un nouveau après un
// blocage "injoignable") — mémorisé, survit au prochain import du fichier.
export async function pushContact(db, refs, contact) {
  if (!db) return;
  await fetch('/api/tickets', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ refs, contact }),
  });
}
