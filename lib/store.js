...
export async function loadInitial() {
  try {
    ...
    if (s.db && Array.isArray(s.techs)) {
      let dayData = { tickets: [], assign: {}, reports: {}, statuts: {} };
      try {
        const d = await fetch(`/api/tickets?day=${today()}`, { cache: 'no-store' }).then((r) => r.json());
        if (d.db) dayData = { tickets: (d.tickets || []).map(fromRow), assign: d.assign || {}, reports: d.reports || {}, statuts: d.statuts || {} };
      } catch {}
      return { db: true, techs: s.techs, zones: s.zones, chefs: s.chefs, ...dayData };
    }
  } catch {}
  return { db: false, ...localSettings(), tickets: [], assign: {}, reports: {}, statuts: {} };
}
...
// L'orienteur remet un ticket arbitré dans le dispatch du jour.
export async function pushPlanifier(db, refs, dedans = true) {
  if (!db) return;
  await fetch('/api/tickets', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ refs, planifier: dedans, day: today() }),
  });
}
...
export async function pushStatut(db, refs, statut, motif) {
  if (!db) return;
  const item = { refs, statut: statut || null, motif: motif || null, day: today() };
