...
import { normSro } from './na-dispatch.js';
...
async function init() {
  ...
  await sql`create table if not exists na_sro (
    sro_key text primary key,
    sro     text not null,
...
// ── Réglages : 4 équipes NA + correspondance SRO → équipe ──────
export async function getSettings() {
  await ensureSchema();
  const sql = db();
  const [teams, sro] = await Promise.all([
    sql`select name, phone, active from na_teams order by ord, name`,
    sql`select sro, team from na_sro order by ord, sro`,
  ]);
  return { teams, sro };
}

export async function saveSettings({ teams = [], sro = [] }) {
  await ensureSchema();
  const sql = db();
  const cTeams = teams.filter((t) => String(t.name || '').trim());
  const cSro = sro.filter((z) => String(z.sro || '').trim());

  await sql.transaction([
    sql`delete from na_teams`,
    sql`insert into na_teams (name, phone, active, ord) select * from unnest(
      ${cTeams.map((t) => t.name.trim())}::text[],
      ${cTeams.map((t) => t.phone || '')}::text[],
      ${cTeams.map((t) => t.active !== false)}::boolean[],
      ${cTeams.map((_, i) => i)}::int[]
    ) on conflict (name) do nothing`,
    sql`delete from na_sro`,
    sql`insert into na_sro (sro_key, sro, team, ord) select * from unnest(
      ${cSro.map((z) => normSro(z.sro))}::text[],
      ${cSro.map((z) => z.sro.trim())}::text[],
      ${cSro.map((z) => z.team || '')}::text[],
      ${cSro.map((_, i) => i)}::int[]
    ) on conflict (sro_key) do update set sro = excluded.sro, team = excluded.team, ord = excluded.ord`,
  ]);
  return getSettings();
}
...
async function suggestMap() {
  const sql = db();
  const [sro, teams] = await Promise.all([
    sql`select sro_key, team from na_sro`,
    sql`select name from na_teams where active`,
  ]);
  const active = new Set(teams.map((t) => t.name));
  const map = {};
  for (const z of sro) if (z.team && active.has(z.team)) map[z.sro_key] = z.team;
  return map;
}
...
export async function importCommandes(commandes, { day, avecStatutDepart = false } = {}) {
  ...
  // cours (hors fait/annulé/blocage), en piochant les plus anciennes (date de
  // réception) parmi les commandes non affectées de son SRO. Une commande
  // déjà affectée (manuellement ou par un dispatch precedent) n'est jamais
  // réattribuée.
  const CAPACITE = 10;
  ...
  const chargeParEquipe = await sql`
    ...
    from na_commandes
    where assigned_team is not null and statut is null
...
export const STATUTS = ['fait', 'annule', 'blocage'];
