...
export function parseTickets(rows) {
  ...
  const col = {
    ...
    // "ZR" (Zone Répartiteur) apparaît dans certains exports à la place de
    // OLT/MSAN — même rôle (clé de zone pour le dispatch par équipe), un nom
    // différent. On ne bascule dessus que si msan/olt n'existe pas.
    msan: findCol(headers, 'msan', 'olt', 'zr'),
    ...
    // Certains fichiers (nouveau format GPON) renomment la colonne
    // "Avancement" en "Splitter" — elle contient le même texte de
    // progression (« En cours », « Planifié », « SPLT ... ISOLE »...),
    // c'est de là que le regroupement splitter est dérivé. On ne bascule
    // sur ce repli que si aucune colonne "avancement" n'existe, pour ne
    // rien changer aux fichiers au format habituel.
    avancement: findCol(headers, 'avancement', 'splitter'),
