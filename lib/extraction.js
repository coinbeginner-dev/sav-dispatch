...
// Annulation d'une saisie : remet le ticket « en attente ».
// Indispensable quand le chef s'est trompé de numéro — sans ça, le ticket
// visé par erreur sort de la vigilance alors qu'il court vers la pénalité.
const MOTS_ANNULATION = [
  'annuler', 'annule', 'annulee', 'annulation', 'erreur', 'errone', 'errone',
  'supprimer', 'supprime', 'oublie', 'oublier', 'ignore', 'ignorer',
  'ghalat', 'ghlat', 'smhli', 'saheb',
];

export function estAnnulation(texte) {
  const mots = new Set(normaliser(texte).split(' '));
  return MOTS_ANNULATION.some((m) => mots.has(m));
}
...
export function analyser(texte, tickets) {
  ...
  for (const ligne of String(texte || '').split(/[\n;]+/).map((l) => l.trim()).filter(Boolean)) {
    ...
    // L'annulation prime : « R341024219 fait, erreur » veut dire annuler.
    const annulation = estAnnulation(ligne);
    const statut = annulation ? null : detecterStatut(ligne);
    for (const ref of refs) {
      instructions.push({ ref, ticket: parRef.get(ref), statut, annulation, note: ligne });
    }
  }
  return { instructions, ignorees, horsPerimetre: [] };
}
...
export function messageConfirmation(res, typeMessage = 'text') {
  ...
  const lignes = [];
  const annulations = res.instructions.filter((i) => i.annulation);
  const prets = res.instructions.filter((i) => !i.annulation && i.statut);
  const sansStatut = res.instructions.filter((i) => !i.annulation && !i.statut);

  for (const i of annulations) {
    const avant = i.ticket.statut ? ` (${LIBELLE[i.ticket.statut]} annulé)` : '';
    lignes.push(`↩️ ${i.ref} — ${i.ticket.client} : remis en attente${avant}`);
  }
  for (const i of prets) {
    // On rappelle l'ancienne valeur : c'est ainsi que le chef repère
    // qu'il vient d'écraser une saisie faite plus tôt, par lui ou par l'orienteur.
    const avant = i.ticket.statut && i.ticket.statut !== i.statut
      ? ` (remplace : ${LIBELLE[i.ticket.statut]})` : '';
    lignes.push(`✅ ${i.ref} — ${i.ticket.client} : ${LIBELLE[i.statut]}${avant}`);
  }
  for (const i of sansStatut) {
    lignes.push(`${i.ref} — ${i.ticket.client} : fait, pas d’accès, ou reporté ?`);
  }
  for (const ref of res.horsPerimetre || []) {
    lignes.push(`${ref} n’est pas attribué à tes équipes aujourd’hui — rien enregistré.`);
  }
  ...
  if (prets.length || annulations.length) {
    lignes.push('Enregistré. Pour corriger : renvoie le numéro avec le bon statut, '
      + 'ou « R… annuler » pour remettre en attente.');
  }
  return lignes.join('\n');
}
